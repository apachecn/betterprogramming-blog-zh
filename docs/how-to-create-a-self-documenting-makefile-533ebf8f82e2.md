...
> 原文：<https://betterprogramming.pub/how-to-create-a-self-documenting-makefile-533ebf8f82e2>

## 使用您可以签入的命令别名加速您的开发操作

![](img/3ffb975b63bdf9842f9b541a7b6f91cb.png)
...
我每天都使用`make`来消除日常开发活动中的乏味，比如更新程序、安装依赖项和测试。为了用 Makefile (GNU make)完成所有这些，我们使用了 [Makefile 规则](https://www.gnu.org/software/make/manual/make.html#Rules)和[方法](https://www.gnu.org/software/make/manual/make.html#Recipes)。POSIX 风格 make 也有类似的相似之处，比如[目标规则](https://pubs.opengroup.org/onlinepubs/9699919799/utilities/make.html#tag_20_76_13_04)；这里有一篇关于兼容 POSIX 的 Makefiles 的文章。

下面是一些我们可以`make`更容易(抱歉)的事情的例子:
...
现在我们有一些命令行别名，您可以签入！好主意！如果你想知道奇怪的`##`注释语法是怎么回事，它会变得更好。

# 自我记录的 Makefile

别名很棒——如果你不用不停地输入`cat Makefile`就能记住它们是什么和它们做什么。自然，你需要一个`help`命令:
...
我们将把它放在文件的顶部，这样它就是默认的目标。现在要查看我们所有方便的快捷方式和它们的功能，我们只需运行`make`，或`make help`:
