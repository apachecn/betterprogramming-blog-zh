...
每种语言中最常被滥用的递归例子是计算一个数的阶乘。对于 Python 开发人员来说，这是一个愚蠢的例子，因为 Python 在其数学库中包含了一个阶乘函数，其性能超过了任何用 Python 原生编写的函数。但是阶乘函数很简单，很容易理解:
