# 如何在一个完全远程的世界中构建可扩展的现代 Web 应用程序
...
我们的工程师都同意，在构建现代 web 应用程序时，远程协作有三个常见的陷阱。
...
缓存是可伸缩性最好的朋友。如果您的远程开发团队正在开发任何类型的面向公众的 web 服务，他们应该认真考虑实现某种形式的缓存，以避免每次都必须重新呈现页面。缓存降低了数据加载成本，允许更快的数据检索，并且消除了返回原始数据库的需要。
